...
また、Groongaで高速に全文検索するためには語彙表と呼ばれる酒類のテーブルを別に作成する必要があります。このテーブルの定義は以下の通りです。ここではそういうものかと思って進んでください。

//table[termstable][Termsテーブルの定義]{
_key	content_terms
-------------------
検索キーワードに対応するキーワードです	@<code>{Documents}テーブルの@<code>{content}カラムのうち、どのレコードを参照しているかを示す物です
//}
...
細かいパラメーターの意味はここでは気にしないで構いません。後で解説します。ここで考慮したほうがよいパラメーターはinit.jsの冒頭のにある次の三つです。

: @<code>{GROONGA_ORIGIN}
  groonga-httpdにアクセスするためのオリジンです。@<code>{docker run}した際にポート番号を変更している場合は、ここも変更してください。
: @<code>{CONTENT_TABLE}
