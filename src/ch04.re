...
//list[ch04/upload/index.php][index.php][php]{
#@mapfile(ch04/upload/index.php)
#@end
//}

//list[ch04/upload/upload.php][upload.php][php]{
#@mapfile(ch04/upload/upload.php)
#@end
//}
...
また、エラーが表示されてしまった場合などPHPのでバッグが必要な場合は、本書末尾の@<hd>{appendix|PHPのデバッグ}にデバッグ方法を記していますので、参考にされてください。

//footnote[multi-select][ファイル選択ダイアログでCtrlキーを押しながらファイルをクリックしていくことで、選んだファイルを同時にアップロードできます。また、Shiftを押しながら最初のファイルと最後のファイルをクリックすることで、その間にあるファイルをすべて選択状態にできます。なお、PHPの設定により、合計で1GiBを超えるアップロードは失敗します。]
...
//list[ch04/extract/upload.php][upload.php][php]{
<?php
...
//list[ch04/insert/table.php][table.php][php]{
#@mapfile(ch04/insert/table.php)
#@end
//}
