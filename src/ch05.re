...
一単語で検索している間は気になりませんが、検索フォームに二単語入れると、検索結果に違和感を覚えるかも知れません。例えば「cat MySQL」で検索した場合に、「catは含んでいるが、MySQLは含んでいない」という文書もヒットする（OR検索になっている）ためです。こういう結果が望ましい場面もありますが、Google検索はこうではなく「cat MySQL」で検索した場合には「catもMySQLも両方とも含んでいるページ」が検索結果上位に登場するようになっています（AND検索になっている）。Google検索には非常にたくさんの人が馴染んでいるため、どちらの挙動でもいい場合には合わせておくのがいいでしょう。
...
AGAINST('*D+ cat Ruby' IN BOOLEAN MODE)
//}
